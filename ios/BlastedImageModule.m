...
RCT_EXPORT_METHOD(loadImage:(NSString *)imageUrl headers:(NSDictionary *)headers resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
        ...
        if (error) {
            ...
            switch (cacheType) {
                case SDImageCacheTypeMemory:
                    message = [NSString stringWithFormat:@"(MEMORY1) %@", imageURL];
                    break;
                case SDImageCacheTypeDisk:
                    message = [NSString stringWithFormat:@"(DISK1) %@", imageURL];
                    break;
                default:
                    message = [NSString stringWithFormat:@"(NETWORK1) %@", imageURL];
                    break;
            }
